...
class PictureModel extends BaseModel{
    ...
    private function _dealPicture($temp_name)
    {
        ...
        if ($waterOpen) {
            ...
            if ($waterImage) {
                ...
                $water = think_get_root().'/public/static/backstage/images/water.png';
            }
     ...
     * @return array
     */
    public function getPictureList($r=10)
    {
        $map['status']=['in','0,1'];
        $list = $this->where($map)->order('create_time desc')->paginate($r,false);
        $page = $list->render();
        $data = $list->toArray();
