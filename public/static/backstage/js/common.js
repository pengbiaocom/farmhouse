...
$(function () {
    ...
    layui.config({
        base:window.Think.PUBLIC+ '/layui/lay/modules/'
    ...
    $('.ajax-post').click(function () {
        ...
        if (($(this).attr('type') == 'submit') || (target = $(this).attr('href')) || (target = $(this).attr('url'))) {
            ...
            $.post(target, query).success(function (data) {
                if (data.code == 1) {
                    if (data.url) {
                        toast.success(data.msg + ' 页面即将自动跳转~');
                    } else {
...
function highlight_subnav(url) {
    $('#sub_menu').find('a[href="' + url + '"]').closest('dd').addClass('layui-this');
}

moduleManager = {
    'install': function (id) {
        $.post(U('backstage/module/install'),{id:id},function(msg){
            handleAjax(msg);
        })
    },
    'uninstall': function (id) {
        $.post(U('backstage/module/uninstall'),{id:id},function(msg){
            handleAjax(msg);
        })
 ...
 * @param height
 */
function layer_open(url,title,width,height){
    if(width==null){
        width = '500px';
    }
    if(height==null){
